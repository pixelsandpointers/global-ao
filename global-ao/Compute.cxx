...
void AOCompute::run(BVH &bvh, int numIters, int vert_batchSize, int tri_batchSize)
{
    GLint numSamples[3];
    glUseProgram(ID);
    ...
    if (useBVH)
    {
        ...
        for (size_t i = 0; i < numIters; ++i){
            ...
            for (int tri_offset = 0; tri_offset < bvh.tris.size(); tri_offset+=tri_batchSize){
                size_t n_tris = std::min(size_t(tri_batchSize), size_t(std::max(int(bvh.tris.size())-int(tri_offset), 0)));
                ...
                for (int vert_offset = 0; vert_offset < bvh.verts_pos.size(); vert_offset+=vert_batchSize){
                    size_t n_verts = std::min(size_t(vert_batchSize), size_t(std::max(int(bvh.verts_pos.size())-int(vert_offset), 0)));
                    ...
                    std::cout << "Dispatched Compute: " << (vert_offset/vert_batchSize) << "/" << bvh.verts_pos.size()/vert_batchSize+1;
